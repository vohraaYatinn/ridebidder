  ...
  return (
    <Card variant="glass" className="w-full overflow-hidden animate-scale-in">
      ...
      <div className="pt-16 pb-6 px-5 text-center">
        <h2 className="text-xl font-bold">{profile?.user?.first_name}</h2>
        
        <div className="flex items-center justify-center mt-1 mb-3">
          <Star className="h-4 w-4 text-yellow-400 fill-yellow-400 mr-1" />
          <span className="text-sm font-medium">{profile?.total_score/profile?.total_ratings}</span>
          <span className="mx-2 text-muted-foreground">•</span>
          <span className="text-sm text-muted-foreground">{profile?.total_trip} Rides</span>
        </div>
        
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div className="flex items-center">
            <Mail className="h-4 w-4 text-blue-400 mr-2" />
            <span className="text-sm truncate">{profile?.user?.email?profile?.user?.email:'N/A'}</span>
          </div>
          <div className="flex items-center">
            <Phone className="h-4 w-4 text-blue-400 mr-2" />
            <span className="text-sm">{profile?.user?.phone_number?profile?.user?.phone_number:'N/A'}</span>
          </div>
